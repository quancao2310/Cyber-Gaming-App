...
BEGIN
  ...
  CREATE TEMPORARY TABLE IF NOT EXISTS selected_device AS (
    SELECT room_type, room_order, slot_order, device_order, name, start_date, expire_time, last_time_maintain
    FROM device
  ...
  CREATE TEMPORARY TABLE IF NOT EXISTS slots_invoices AS (
    ...
    GROUP BY room_type, room_order, slot_order, device_order, name, start_date, expire_time, last_time_maintain
